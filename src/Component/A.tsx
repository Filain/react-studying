...
prev	            - дозволяє оновлювати параметри запиту на основі попередніх значень. У цьому випадку prev допомагає забезпечити коректну роботу з поточними параметрами.

setSearchParams(prev => {
    params.set('query', query); // Заміна значення параметра 'query'
    return params;};

setSearchParams(prev => {
    params.append('category', 'newCategory'); // Додавання нового значення до параметра 'category'
    return params; };
const query = searchParams.get('query') || '';  //Отримати значення 'query
const categories = searchParams.getAll('category'); // повертає масив усіх значень параметра category
